...
/**
 * Self-building model — no "Build 3D" button.
 *
 * This app IS a 3D model builder, so a button that asks permission to build the
 * model is asking to do the one thing the app is for. Instead the model rebuilds
 * itself whenever the traced content settles: trace a wall, lay a floor — the 3D
 * updates on its own. Everything stays editable and undoable, so there's nothing
 * to confirm. (Roofs are the one exception: buildModel never auto-adds one — a
 * roof is a deliberate step the user traces.)
 *
...
function contentSignature(state: ReturnType<typeof useAppStore.getState>): string {
  ...
  for (const a of [...state.floorsAreas, ...state.roofAreas]) {
    // Skip auto-derived slabs/decks: they're a pure function of the walls (already
    // folded in above) and get FRESH ids on every build, so including them would
    // make each auto-build change the signature and trigger the next — a rebuild
    // loop. Only user-traced areas (stable ids) count as new input.
    if (a.id.startsWith('auto-')) continue
...
export function useAutoBuild(): void {
  ...
  useEffect(() => {
    // Seed from the current state so mounting doesn't trigger a build of whatever
    // was already there (e.g. a restored project).
